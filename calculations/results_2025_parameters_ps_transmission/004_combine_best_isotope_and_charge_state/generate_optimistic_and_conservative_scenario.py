...
full_result_baseline = pd.read_csv('../001_baseline_scenarios/output/output_csv/1_baseline_ps_rest_gas.csv', index_col=0)
...
for i, ion_type in enumerate(ion_data.columns):
        ...
        read_str = '../002_charge_state_scan/output/charge_scan_results/{}_2_no_PS_splitting_{}_ps_rest_gas.csv'.format(ion_type, output_extra_str)       
...
df_optimistic.T.to_csv('output/2_no_PS_bunch_splitting_optimistic_ps_rest_gas.csv')
full_result_baseline.to_csv('output/1_baseline_baseline_ps_rest_gas.csv')
...
df_SC_and_max_intensity.to_csv('output/2_no_PS_bunch_splitting_optimistic_for_paper_ps_rest_gas.csv', index=True)
...
fig3.savefig('output/Baseline_Optimistic_vs_WG5_scecnario_ps_rest_gas.png', dpi=250)
plt.close()